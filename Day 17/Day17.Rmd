...
I got this to work, found the answer, was disappointed that it took the better part of an hour, tried to refine it, broke it.
It now takes only a few seconds to come up with the wrong answer.  (All the sample problems I've found so far work.) Also, unfortunately, my notes are such a hash on the first go-round that I honestly can't tell what's broken.
...
2. Checks:
 + have we been there before? if so, back to 1
 + is this the end? if so, stop. (because the check is the first thing done, there's no way that something with a lower heat loss could have happened earlier)
3. Find all the possible moves from that spot (if we got there going left/right, the moves are up/down & vice versa.)
4. Add them to the queue where priority = heatloss to that spot+manhattan distance (which is the lowest possible heatloss - assuming it found a trail of all 1s from that point forward)
end repeat
...
lowestheatloss<-function(startx=1,starty=1,endx=13,endy=13,mindist=1,maxdist=3,lf=lavafield){
  ...
  while(lavaqueue$size()>0){
    ...
    if(currentx==endx&&currenty==endy){
      out=currentheat
      break}
    if(beenthere$has(paste0(orientation,currentx,currenty))){next
      }else{beenthere$set(key=paste0(orientation,currentx,currenty),value=paste(currentstep))}
    newheat<-currentheat
    newx<-currentx
    newy<-currenty
...
Wrong answer, but fast (for me) wrong answer!
